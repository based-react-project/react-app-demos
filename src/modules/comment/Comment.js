...
class Comment extends Component {
  ...
  _getProcessedContent(content) { // 要把 `` 包含的内容用 <code> 包裹起来，一个正则表达式就可以做到了：
    return content
      .replace(/&/g, "&amp;")
      .replace(/</g, "&lt;")
      .replace(/>/g, "&gt;")
      .replace(/"/g, "&quot;")
      .replace(/'/g, "&#039;")
      .replace(/`([\S\s]+?)`/g, '<code>$1</code>') // 前 5 个 replace 实际上是把类似于 <、> 这种内容替换转义一下，防止用户输入 HTML 标签。最后一行代码才是实现功能的代码
  }
  handleDeleteComment() {
